...
// ../.wrangler/tmp/bundle-yANpZw/checked-fetch.js
  ...
  try {
    if (request.method === "GET") {
      ...
      try {
        ...
        console.error("Orders GET Query Error:", queryErr);
        return Response.json([]);
      }
    }
    if (request.method === "POST") {
  ...
  try {
    if (request.method === "GET") {
      ...
      try {
        ...
        console.error("Products GET Query Error:", queryErr);
...
var onRequest6 = /* @__PURE__ */ __name(async (context) => {
  ...
  if (request.method === "GET") {
    try {
      if (!env.DB) return Response.json(mockTags);
      const { results } = await env.DB.prepare("SELECT * FROM tags ORDER BY name ASC").all();
      return Response.json(results);
    } catch (e) {
      console.error("Tags GET Error:", e.message);
      return Response.json(mockTags);
    }
  }
  if (request.method === "POST") {
    try {
      if (!env.DB) return Response.json({ success: true, id: "mock-tag-id" });
      const data = await request.json();
  ...
  try {
    if (request.method === "GET") {
      ...
      try {
        ...
        console.error("Users GET Query Error:", queryErr);
...
// ../.wrangler/tmp/pages-fW6Axr/functionsRoutes-0.6016327669457331.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-yANpZw/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-yANpZw/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7517682295368795.mjs.map
